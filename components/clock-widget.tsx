...
function AnalogClockWidget({
  ...
  const [format, setFormat] = React.useState<"12h" | "24h">(
    "12h"
  );
      ...
      <div className="flex items-center gap-10">
        ...
        >
          ...
          {numbers.map((num, i) => {
            ...
            const x =
              50 + config.radius * Math.cos(angle);

            const y =
              50 + config.radius * Math.sin(angle);

            return (
              <span
                key={num}
                className={cn(
                  "absolute text-white/70 font-light",
                  config.numbers
                )}
                style={{
                  left: `${x}%`,
          ...
          {/* Second hand — ORANGE */}
          <div
            className="absolute left-1/2 top-1/2 w-px bg-orange-500 origin-bottom"
            style={{
              height:
                size === "lg"
                  ? "40px"
        ...
        <div className="flex flex-col gap-2 min-w-0">
          ...
          <div className="flex items-baseline gap-1">
            <span className="font-mono text-2xl md:text-3xl text-white tabular-nums whitespace-nowrap">
              {formattedTime}
            </span>
          ...
          <div className="font-mono text-[9px] uppercase tracking-widest text-white/30 truncate">
            {timeZone}
          </div>

          {/* 12H / 24H */}
          <div className="flex w-fit items-center rounded-md border border-white/20 overflow-hidden">
            <button
              type="button"
              onClick={() => setFormat("12h")}
              className={cn(
                "px-2.5 py-1 font-mono text-[9px] uppercase transition-colors",
                format === "12h"
                  ? "bg-white text-black"
...
/* =========================================================
   EVERYTHING BELOW IS YOUR ORIGINAL EINUI COMPONENTS
========================================================= */
...
function WorldClockWidget({
  ...
  return (
    <GlassWidgetBase
      className={cn("min-w-45", className)}
      glowColor="blue"
    >
      <div className="space-y-3">
        {clocks.map((clock, i) => (
          ...
          >
            ...
            <span className="text-white/80 text-lg tabular-nums">
              {times[i] || "--:--"}
            </span>
          </div>
        ))}
